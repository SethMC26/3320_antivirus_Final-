...
void set_thread(int threads)
{
    ...
    {
        log_message(LL_ERROR, "Invalid number: %d", threads);
    }
}
 ...
 */
int scan_dir(char *target_dir, int num_threads)
{
    ...
    int *thread_indices = malloc(num_threads * sizeof(int));
    ...
    while ((entry = readdir(dir)) != NULL)
    {
        ...
        if (local_active_threads >= num_threads)
        {
 ...
 * @return int 0 if success, 1 if error
 */
int scan_system(int num_threads)
{
    log_message(LL_INFO, "Starting system scan...");
    int result = scan_dir("/", num_threads); // Scan from root directory
    if (result == 0)
    {
